...
def set_cookies(browser,url):
    ...
    browser.add_cookie({'name':'aep_usuc_f',
                        'value':'isfm=y&site=glo&c_tp=GBP&x_alimid=814969923&isb=y&ups_u_t=1578293226431&region=UK&b_locale=en_US&ae_u_p_s=0'})
    browser.get(url)
...
def crawling_to_findall_pages(browser,url_pattern):
    ...
    while not isLastPage:
        ...
        next_button = browser.find_element_by_css_selector('div.ui-pagination-navi.util-left a.ui-pagination-next')
        isLastPage = next_button.get_attribute('class').find('disabled')>-1
        dataframe.to_excel('home-decoration.xlsx')


    #dataframe.to_excel('custom-wall-sticker1.xlsx')
...
initial_url_pattern = 'https://www.aliexpress.com/store/group/Custom-made-Wall-Stickers/1225042_259917284/{page}.html'
...
initial_url = initial_url_pattern.replace('{page}','27')
